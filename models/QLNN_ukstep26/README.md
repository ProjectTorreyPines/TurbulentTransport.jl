...
TGLF-trained QL-weight heads for the UKAEA STEP design point (STEP TGLF corpus, rho = 0.1..0.9,
Aug 2026). 20-member ensembles per head.
...
(e, DT, imp) lumping** of those inputs (IMAS bulk/impurity convention). 35 inputs incl.
`MASS_2` (density-weighted D/T mass, 1.00 pure D
...
QL-weight heads: Spearman 0.74-0.86 on every apar/bpar channel. Eigenvalue gamma R² 0.945,
width 0.981, stability accuracy 0.968 / F1 0.982. Momentum is the weakest head.
...
largest fluxes are underpredicted (summed Qe ratio 0.53).
